...
- 同一工作区的检查共享一个原子锁，保护构建和测试产物。相同 Plan、输入、环境和命令可复用成功记录。活动进程返回 pending 非零状态；失去心跳且不属于存活 runner 的运行变为 `orphaned`。命令有总时限，记录当前步骤与最后输出时间；`run cancel` 只对本机的指定活动运行写取消请求，由运行所有者停止其进程组。坏记录单独标记，不阻断整个运行列表。
...
运行清单的 `check_policy_version` 固定本次命令选择规则。没有此字段的 schema 2 历史清单按策略 1 解释；旧策略 2 仍按历史命令校验。新运行使用策略 3：集成检查补齐 Plan 显式映射的测试；发布检查要求同输入的集成通过记录，只执行生产构建与生产态 E2E，不再重复整套 `verify`。当前 Plan 完成必须使用当前策略，并核对本地运行记录与日志摘要。历史记录仍按原策略解释。
