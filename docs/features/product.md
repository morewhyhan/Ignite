...
- R15（REQ-PRODUCT-015）：验收必须核对真实发现和执行的测试，证据必须来自可追溯的运行，环境或执行输入变化应使相关结果失效；仅提交证据或派生状态不应迫使同一输入重复集成检查。关键私有资源访问要用故障注入证明测试会因权限过滤失效而失败。
...
- R17（REQ-PRODUCT-017）：模板工作区只保留供后续开发使用的文档真源、空模板和当前基线交付记录；建设 Ignite 时形成的旧 Plan、Release、运行证据和整改报告只留在 Git 历史，不作为衍生项目的当前上下文。
...
- AC-PRODUCT-015（REQ-PRODUCT-017）：Given 复制模板的初始工作区，When 扫描 Plans、Releases、证据和根目录，Then 不包含建设期遗留记录，且当前基线的证据可追溯。
