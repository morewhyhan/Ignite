...
| 迁移契约 | `scripts/test-migrations.mjs`、`pnpm ignite validate` | 干净数据库部署、重复部署、schema drift 与带样本数据的连续迁移 |
...
Vitest 的测试文件使用最多两个隔离 worker 并行；每个文件内的用例保持原有顺序，临时 Git 仓库和数据库按用例独立创建。涉及外键、鉴权或私有资源的关键测试要做有针对性的故障注入：暂时移除所有权过滤后，对应测试必须失败。测试替身应按生产查询条件工作，不能在替身中偷偷补回生产代码漏掉的过滤。Playwright 仍使用单 worker，保护同一临时数据库。
